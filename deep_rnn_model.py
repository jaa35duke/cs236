...
class DeepRnnModel(BaseModel):
    ...
    def __init__(self, config):
        ...
        self._center = tf.compat.v1.get_variable('center',shape=[num_inputs],trainable=False)
        self._scale  = tf.compat.v1.get_variable('scale',shape=[num_inputs],trainable=False)

        batch_size = self._batch_size = tf.compat.v1.placeholder(tf.int32, shape=[])
        self._seq_lengths = tf.compat.v1.placeholder(tf.int32, shape=[None])
        self._keep_prob = tf.compat.v1.placeholder(tf.float32, shape=[])
        self._phase = tf.compat.v1.placeholder(tf.bool, name='phase')

        self._inputs = list()
        self._targets = list()

        for _ in range(max_unrollings):
            inp = tf.compat.v1.placeholder(tf.float32, shape=[None,num_inputs])
            tar =  tf.compat.v1.placeholder(tf.float32, shape=[None,num_outputs])
        ...
        def rnn_cell():
            ...
            if config.rnn_cell == 'gru':
                cell = tf.compat.v1.nn.rnn_cell.GRUCell(num_hidden)
            elif config.rnn_cell == 'lstm':
                #cell = tf.compat.v1.nn.rnn_cell.LayerNormBasicLSTMCell(num_hidden, # change tf.rnn. to tf.contrib.rnn for tf 1.0
        ...
        stacked_rnn = tf.compat.v1.nn.rnn_cell.MultiRNNCell([rnn_cell() 
                                                   for _ in range(config.num_layers)])

        rnn_outputs, state = tf.compat.v1.nn.static_rnn(stacked_rnn,
                                                       self._scaled_inputs,
                                                       dtype=tf.float32,
                                                       sequence_length=self._seq_lengths)

        self._w = output_w = tf.compat.v1.get_variable("output_w", [num_hidden, num_outputs])
        output_b = tf.compat.v1.get_variable("output_b", [num_outputs])

        self._outputs = list()
        for i in range(max_unrollings):
            self._outputs.append( tf.compat.v1.nn.xw_plus_b( rnn_outputs[i], output_w, output_b ) )
        ...
        reversed_outputs = tf.reverse_sequence(input=outputs,
                              seq_lengths=self._seq_lengths,seq_axis=1,batch_axis=0)

        reversed_targets = tf.reverse_sequence(input=targets,
                              seq_lengths=self._seq_lengths,seq_axis=1,batch_axis=0)
        ...
        if config.backfill is True:
            self._mse_0 = tf.compat.v1.losses.mean_squared_error(last_target[:,ktidx], last_output[:,ktidx])
            self._mse_1 = tf.compat.v1.losses.mean_squared_error(last_target, last_output)
        else:
        ...
        self._mse = tf.compat.v1.losses.mean_squared_error(last_target[:,ktidx], last_output[:,ktidx])
        #if config.data_scaler is not None and config.scale_targets is True:
        ...
        print("------")
        if hasattr(tf.compat.v1.train,config.optimizer):
            #optimizer = getattr(tf.train, config.optimizer)(learning_rate=self._lr,**args)
            optimizer = tf.compat.v1.train.AdadeltaOptimizer(learning_rate=self._lr, **args)
        else:
    ...
    def _mean_squared_error(self, targets, outputs, mask):
        ...
        mse = tf.reduce_sum( input_tensor=loss ) / tf.reduce_sum( input_tensor=mask )
        return mse
